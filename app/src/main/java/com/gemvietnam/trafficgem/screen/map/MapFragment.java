...
//    private double delLon = 0.0088355;
    ...
    private void processGet() {
        if(!AppUtils.networkOk(getViewContext())){      // check internet
            Toast.makeText(getViewContext(), "NO INTERNET !!!", Toast.LENGTH_LONG).show();
            return;
        }
        String period_time = sPeriod.getSelectedItem().toString();  // get period_time when user select
        String timeValue = "10";
        ...
        User user = Hawk.get(LAST_USER);
            // perform request get report
        DataExchange dataExchange = new DataExchange();
        String response = dataExchange.getReport(user.getToken(), timeValue);
        Location location = getLastKnownLocation();     // get curent location
        if(location == null)    return;
            // set border of map
        double north = location.getLatitude(), west = location.getLongitude(),
                east = location.getLongitude(), south = location.getLatitude();
        GetReportResponse getReportResponse = new GetReportResponse(response);
        getReportResponse.analysis();       // analysis response
    ...
    private void showTrafficState() {
        ...
        User user = Hawk.get(LAST_USER);
            // perform request get traffic data
        DataExchange trafficState = new DataExchange();
        getLayer1 = trafficState.getCurrent(user.getToken(), 1);    // color layer 1
        getLayer2 = trafficState.getCurrent(user.getToken(), 2);    // color layer 2
        getLayer3 = trafficState.getCurrent(user.getToken(), 3);    // color layer 3
    ...
    private void setTrafficState(int layer){
        ...
        points = new Point[height][width];  // set size of the box in map
        colors = trafficResponse.getColorArray();   // set color of box in  map
...
//    hideProgress();
        ...
        if(trafficStateFlag){       // flag : using to " traffic state " function
            googleMap.setOnCameraIdleListener(new GoogleMap.OnCameraIdleListener() {
                @Override
                public void onCameraIdle() {
                    int zoomLevel = Math.round(mMap.getCameraPosition().zoom);  // event zoom
                    ...
                    if(layerTemp[0] != LAYER){
                        layerTemp[0] = LAYER;
                        mMap.clear();
                        setTrafficState(LAYER); // Each layer has a zoom layer
                    }
                }
            });
        }
